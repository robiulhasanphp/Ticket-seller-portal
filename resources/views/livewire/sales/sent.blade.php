    ...
    <div class="sm:col-span-6 px-8">
        <div class="py-5">
            <div id="cards" class="flex border py-2  pr-10">
                ...
                <p class="px-4 text-base text-black pt-2 font-semibold md:text-xl">“Meine Aufträge -Versendet ”</p>
            </div>
        </div>
    </div>
            ...
            <div class="mt-2 flex flex-col">
                <div class="min-w-full overflow-x-auto align-middle shadow sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        @foreach ($saleData as $sent)
                            <tr class="bg-white">
                                @if($sent->ORDR_id  !='')
                                    <td class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="text-sky-500">{{ $sent->ORDR_id }}</span>
                                    </td>
                                @endif
                                @if($sent->detail->ticket->event->name !='')
                                    <td class="whitespace-nowrap py-4  text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{!! $sent->detail->ticket->event->name !!}</span>
                                    </td>
                                @else
                                    <td class="whitespace-nowrap py-4  text-center text-sm text-gray-500"></td>
                                @endif
                                @if($sent->detail->quantity !=null)
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{{ $sent->detail->quantity }}</span>
                                    </td>
                                @else
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500"></td>
                                @endif
                                @if($sent->detail->ticket->category  !=null)
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{{ $sent->detail->ticket->category }}</span>
                                    </td>
                                ...
                                @if($sent['detail']['brokerTotal'] !=null)
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{{ ($sent->detail->brokerTotal != '0.00') ? $sent->detail->brokerTotal : ($sent->detail->brokerTotal - $sent->detail->shippingPrice).toFixed(2) }} {{ $sent->detail->currency }}</span>
                                    </td>
                                @else
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500"></td>
                                @endif
                                <td class="whitespace-nowrap py-4">
                                    <span class="flex justify-center"><button onclick="addLoadingClass()" wire:click="showSalesById({{ $sent->ORDR_id}})"  @click.prevent="menuOpen = true"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
