<div>
    ...
    <div class="sm:col-span-6 px-8">
        <div class="py-5">
            <div id="cards" class="flex border py-2 text-black pr-10">
                ...
                <p class="px-4 text-base text-black pt-2 font-semibold md:text-xl">“Meine Aufträge - Zu versenden”</p>
            </div>
        </div>
    </div>
    ...
    <div class="sm:col-span-6 px-8">
        <div class=" block relative">
            ...
            <input placeholder="Suche nach ID, Event, Ticket ..." wire:model.debounce.500m="search" class="appearance-none rounded-r rounded-l sm:rounded-l-none border border-gray-400 border-b block pl-8 pr-6 py-2 w-full bg-white text-sm placeholder-gray-400 text-gray-700 focus:bg-white focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none"/>
        </div>
    </div>
        ...
        <div class="mx-auto w-full px-4 sm:lg:px-8">
            <div class="mt-2 flex flex-col">
                <div class="min-w-full  align-middle shadow sm:rounded-lg">
                    ...
                    <template  x-if="selectedIds !=''">
                        <button type="button" onclick="addLoadingClass()" x-on:click="$wire.generateShippingLabel(selectedIds)"
                                class="px-10 py-2 bg-red-600  drop-shadow-lg text-xl text-white duration-300 hover:bg-green-600">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                        <tr>
                            <th scope="col" class="overflow-hidden bg-gray-400 px-7 sm:w-12 sm:px-6">
                                <input type="checkbox" name="selectAll" x-on:click="toggleAllCheckboxes()" class="overflow-hidden static left-4 top-1/2  h-4 w-4  text-indigo-600 focus:ring-indigo-600">
                            </th>
                        ...
                        @foreach ($saleData as $send)
                            <tr class="bg-white">
                                @if($send->ORDR_id  !='')

                                <td class=" px-7 sm:w-12 sm:px-6">
                                    <input type="checkbox" name="ids" value="{{$send->ORDR_id}}" x-model="selectedIds" class="static left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
                                </td>
                                @endif
                                @if($send->ORDR_id  !='')
                                    <td class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="text-sky-500">{{ $send->ORDR_id }}</span>
                                    </td>
                                @endif
                                @if($send->detail->ticket->event->name !='')
                                    <td class="whitespace-nowrap py-4  text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{!! $send->detail->ticket->event->name !!}</span>
                                    </td>
                                @else
                                    <td class="whitespace-nowrap py-4  text-center text-sm text-gray-500"></td>

                                @endif
                                @if($send->detail->quantity !=null)
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{{ $send->detail->quantity }}</span>
                                    </td>
                                @else
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500"></td>
                                @endif
                                @if($send->detail->ticket->category  !=null)
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{{ $send->detail->ticket->category }}</span>
                                    </td>
                                @else
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500"></td>
                                @endif
                                @if($send['detail']['brokerTotal'] !=null)
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500">
                                        <span class="font-medium text-gray-900">{{ ($send->detail->brokerTotal != '0.00') ? $send->detail->brokerTotal : ($send->detail->brokerTotal - $send->detail->shippingPrice).toFixed(2) }} {{ $send->detail->currency }}</span>
                                    </td>
                                @else
                                    <td  class="whitespace-nowrap py-4 text-center text-sm text-gray-500"></td>
                                @endif
                                <td class="whitespace-nowrap py-4">
                                    <span class="flex justify-center"><button onclick="addLoadingClass()" wire:click="showSalesById({{ $send->ORDR_id}})"  @click.prevent="menuOpen = true"> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                                ...
                                                                <li>
                                                                    <ul class="mt-2 text-sm text-gray-800">
                                                                        ...
                                                                        @if($saleDataById[0]['event']['name'] !='')
                                                                            <li class="cursor-default select-none px-4 py-1 font-bold py-5 border-bottom text-fuchsia-700 bg-green-300 rounded-md">{!! $saleDataById[0]['event']['name'] !!}</li>
                                                                        @endif
                                                                        <div class="my-5 py-2 bg-gray-400">
                                                                            <span class="px-4 mb-4 text-white">{{ $saleDataById[0]['detail']['quantity'] }} x {{  $saleDataById[0]['ticket']['category'] }}</span>
                                                                            <div class="py-5">
                                                                                @if($saleDataById[0]['detail']['quantity'] !='')
                                                                                    <li  class="cursor-default select-none px-4 text-white"><span class="font-weight-500 width-120">Preis pro Ticket:</span> {{$saleDataById[0]['detail']['brokerPricePerTicket']}} EUR</li>
                                                                                @endif
                                                                                @if($saleDataById[0]['detail']['brokerTotal'] !='')
                                                                                    <li class="cursor-default select-none px-4 text-white"><span class="font-weight-500 width-120">Gesamtpreis:</span>{{$saleDataById[0]['detail']['brokerTotal']}} EUR</li>
                                                                                @endif
                                                                            </div>
                                                                        </div>
                                                                    </ul>
                                                                ...
                                                                <li>
                                                                    <ul class="my-5 py-2 bg-gray-400 text-sm text-gray-800">
                                                                        ...
                                                                        @if($saleDataById[0]['saleDate']  !='')
                                                                            <li  class="cursor-default select-none px-4 py-1 text-white"><strong>Verkauft am:</strong> {{$saleDataById[0]['saleDate']}}</li>
                                                                        @endif
                                                                        @if($saleDataById[0]['confirmationDate']  !='')
                                                                            <li  class="cursor-default select-none px-4 py-1 text-white"><strong>Bestätigt am:</strong> {{$saleDataById[0]['confirmationDate']}}</li>
                                                                        @endif
                                                                        @if($saleDataById[0]['payDate']  !='')
                                                                            <li  class="cursor-default select-none px-4 py-1 text-white"><strong>Bezahlt am:</strong> {{$saleDataById[0]['payDate']}}</li>
                                                                        @endif
                                                                        @if($saleDataById[0]['shippingDate']  !='')
                                                                            <li  class="cursor-default select-none px-4 py-1 text-white"><strong>Versendet am:</strong> {{$saleDataById[0]['shippingDate']}} {{$saleDataById[0]['detail']['trackingID']}}</li>
                                                                        @endif
                                                                        @if($saleDataById[0]['payOutDate']  !='')
                                                                            <li  class="cursor-default select-none px-4 py-1 text-white"><strong>Ausgezahlt am:</strong> {{$saleDataById[0]['payOutDate']}}</li>
                                                                        @endif
                                                                    </ul>
                                                                </li>
                                                            </ul>
